...
const library = {
  ...
  addReader(readerInfo) {
    const newReader = {
      id: Math.floor(Math.random() * (4000 - 3005) + 3005),
      books: [],
      // весь вміст який вклали у bookInfo
      ...readerInfo,
  ...
  borrowBook(bookId, readerId) {
    ...
    const borrowBook = this.books.find((book) => book.id === bookId);
  ...
  returnBook(bookId, readerId) {
    ...
    book.borrow = false;
    //   person.books - доступ до масиву з книжками користувача
    //  = присвоюʼмо нове значення
    // за допомогою методу filter профільтровуєм масив книжок
    // умова нашого фільтру повернути всі книжки які не мають id який нам передали для видалення
    person.books = person.books.filter((book) => book.id !== bookId);
    ...
    console.log("Книга повернена", operationGivingBack);
  },
...
console.log(library.addBook({ title: "Petro", author: "shoc", year: 2024 }));
console.log(library.addReader({ name: "Petro", email: "email" }));
console.log(library.borrowBook(5, 3001));
console.log(library.borrowBook(4, 3001));
console.log(library.borrowBook(3, 3001));
console.log(library.borrowBook(2, 3001));
console.log(library.returnBook(5, 3001));

console.log(library.getAvailableBooks());

console.log(library.getReaderBooks(3001));
